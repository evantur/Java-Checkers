...
class Data {
    ...
    public String score() { 
        int white, whiteKings, black, blackKings;
        white = whiteKings = black = blackKings = 0;
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
        ...
        String result = "SCORE:%d;%d;%d;%d;";
        result = String.format(result, white, whiteKings, black, blackKings);
        return result;
    }
    ...
    public void makeMove(movesMade move) { // helper method that takes in movesMade type and makes a move
